...
const sectorsData = [
  { mult: 5,  img: '/planet_pink.png',  text: 'x5' },
  { mult: 5,  img: '/planet_green.png', text: 'x5' },
  { mult: 5,  img: '/planet_blue.png',  text: 'x5' },
  { mult: 5,  img: '/planet_cyan.png',  text: 'x5' },
  { mult: 10, img: '/planet_fire.png',  text: 'x10' },
  { mult: 15, img: '/planet_ruby.png',  text: 'x15' },
  { mult: 25, img: '/planet_gold.png',  text: 'x25' },
  { mult: 50, img: '/planet_dark.png',  text: 'x50' },
];
...
async function initPixiEngine() {
  ...
  for (let i = 0; i < sectorsData.length; i++) {
    ...
    highlight.circle(0, 0, 36);
    highlight.stroke({ width: 3, color: 0xfacc15, alpha: 0.95 });
  ...
  function layout() {
    ...
    if (portalSprite) {
      portalSprite.width = minDim * 0.48;
      portalSprite.height = minDim * 0.48;
      portalSprite.x = cx;
      portalSprite.y = cy;
    }
    ...
    // Рассчитываем радиус так, чтобы планеты не выходили за границы высоты и ширины
    const radius = Math.min(minDim * 0.35, (h / 2) - 46);
    const scale = Math.max(0.65, Math.min(1.0, minDim / 440));
    ...
    for (let i = 0; i < planetsArray.length; i++) {
      ...
      if (planetsArray[i].sprite) {
        planetsArray[i].sprite.width = 54 * scale;
        planetsArray[i].sprite.height = 54 * scale;
      }
    ...
    if (winPointer) {
      winPointer.x = cx;
      winPointer.y = Math.max(16, cy - radius - (24 * scale));
    }
  }
...
.history-title { font-size: 11px; font-weight: bold; color: #a855f7; margin-right: 8px; }
.history-mini-list { flex: 1; display: flex; gap: 6px; overflow: hidden; }
.hist-mini-item { display: flex; flex-direction: column; align-items: center; }
.hist-mini-img { width: 18px; height: 18px; mix-blend-mode: screen; }
.hist-mini-mult { font-size: 7px; color: #fff; font-weight: bold; margin-top: 1px; }
.history-more { background: none; border: none; color: #fff; font-size: 10px; cursor: pointer; }
...
.win-glow { filter: drop-shadow(0 0 12px rgba(255, 255, 255, 0.4)); }
.lose-img { filter: grayscale(100%); }
.win-sum { font-size: 32px; font-weight: 900; color: #facc15; margin: 8px 0; }
...
.history-page-title { font-size: 15px; font-weight: bold; }
...
.round-number { font-size: 13px; font-weight: bold; color: #e9d5ff; }
.card-right { display: flex; align-items: center; gap: 12px; }
.history-list-img { width: 30px; height: 30px; mix-blend-mode: screen; }
.history-list-mult { font-size: 14px; font-weight: 900; }
.mult-5 { color: #ec4899; }
.mult-10 { color: #f97316; }
.mult-15 { color: #ef4444; }
.mult-25 { color: #06b6d4; }
.mult-50 { color: #8b5cf6; }
.empty-history { text-align: center; color: #a855f7; margin-top: 30px; font-size: 13px; }
</style>